...
class Kopa_Layout_Box {
	...
	public function output( $use_custom_layout, $option_value, $value ) {				
		...
		$output .= '<label><input class="kopa_use_custom_layout" type="checkbox" name="_use_custom_layout" value="'.esc_attr( $settings['layout'] ).'" '.checked( $settings['layout'], $use_custom_layout, false ).'> '.esc_html__( 'Check if you would like to use custom setting', 'kopatheme' ).'</label>';
		...
		if ( $settings['taxonomy'] ) {
			echo '<tr>';
				echo '<th scope="row" valign="top">';
					echo esc_html__( 'Custom Layout', 'kopatheme' );
				echo '</th>';
				echo '<td>';
		}
