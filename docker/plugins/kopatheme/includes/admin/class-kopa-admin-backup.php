...
class Kopa_Admin_Backup {
	...
	public function import() {
		...
		if ( ! $options || $_FILES['kopa_import_file']['error'] ) {
			Kopa_Admin_Settings::add_error( esc_html__( 'There was a problem importing your settings.', 'kopatheme' ) );
			return false;
		}

		// Make sure this is a valid backup file.
		if ( ! isset( $options['kopathemes-backup-validator'] ) ) {
			Kopa_Admin_Settings::add_error( esc_html__( "The import file you've provided is invalid.", 'kopatheme' ) );
			return false;
		} else {
		...
		if ( $has_updated ) {
			Kopa_Admin_Settings::add_message( esc_html__( 'Settings successfully imported.', 'kopatheme' ) );
		}
