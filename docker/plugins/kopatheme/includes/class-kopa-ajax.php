...
class Kopa_Ajax {
    ...
    public function remove_sidebar() {
        ...
        if ( isset( $sidebars_widgets[ $sidebar_id ] ) && ! empty( $sidebars_widgets[ $sidebar_id ] ) ) {
            $warnings[] = esc_html__( 'This sidebar is currently containing widgets.', 'kopatheme' );
        }
                    ...
                    if ( ! empty( $option_value['sidebars'] ) && is_array( $option_value['sidebars'] ) ) {
                        foreach ( $option_value['sidebars'] as $layout_id => $sidebars ) {
                            ...
                            if ( is_array( $sidebars ) && in_array( $sidebar_id, $sidebars ) ) {
                                ...
                                $errors[] = "&nbsp;&nbsp;&nbsp;&nbsp;" . sprintf( esc_html__( '%1$s of %2$s.', 'kopatheme' ), $layout_name, $page_title );
                            }
                        }
                    }
                }
        ...
        // warning messages
        // if ( ! empty( $warnings ) ) {
        // 	$warnings[] = esc_html__( 'Delete it will also delete all widgets are inside it.', 'kopatheme' );
        // }
        ...
        if ( ! empty( $errors ) ) {
            $allow_delete = false;
            array_unshift( $errors, '<strong>' . esc_html__( 'This sidebar is being used in:', 'kopatheme' ) . '</strong>' );
            $errors[] = '<strong>' . esc_html__('You cannot delete this sidebar.', 'kopatheme') . '</strong>';
        }
        ...
        <div class="kopa-list-of-icon">
            <div class="kopa-row">
                <input type="text"
                       ...
                       onkeyup="Kopa_UI_Icon.filter_icons(event, jQuery(this));">
            </div>
            <div class="kopa-row kopa-wrap">
                <?php foreach($icons as $key => $val): ?>
                <span class="kopa-icon-item kopa-col-xs-2" onclick="Kopa_UI_Icon.select_a_icon(event, jQuery(this));">
